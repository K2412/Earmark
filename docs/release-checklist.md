...
| 9 | Understandable payee rules with previews | ✅ Shipped | `PayeeRuleController`, `PayeeRuleService`; plus history-based auto-categorization (`PayeeHistoryService`) — imports pre-fill the category/bucket last used for a payee, with rules taking precedence | `Household/PayeeRuleManagementTest`, `Household/ImportHistoryCategorizationTest`, `Services/PayeeHistoryServiceTest`, browser `RulesPageTest` |
